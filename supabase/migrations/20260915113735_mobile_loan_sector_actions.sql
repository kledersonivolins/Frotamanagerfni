-- Aprovadores recebem todas as solicitações do próprio setor, mesmo quando o
-- veículo não faz parte da lista pessoal usada para criar novas solicitações.
...
begin
  ...
  with authorized_vehicles as (
    ...
    select l.*
    from public.emprestimos_veiculos l
    ...
    select id from authorized_vehicles
    union
    select veiculo_id from visible
...
-- Transições sensíveis são validadas no servidor. O aplicativo não escolhe o
-- usuário nem o setor autorizador: ambos vêm da sessão autenticada.
...
begin
  ...
  select * into l from public.emprestimos_veiculos
    where id=p_loan_id and tenant=p.tenant and coalesce(ativo,true)
    for update;
