...
begin
  ...
  select jsonb_build_object(
    ...
    'loans',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'checklist_saida_status',l.checklist_saida_status) order by l.data_saida desc,l.id desc)
...
begin
  ...
  if action_name='approve' then
    ...
    if coalesce(l.checklist_saida_obrigatorio,false)
      and lower(coalesce(l.checklist_saida_status,'')) not in ('concluido','concluida','finalizado','finalizada','aprovado','aprovada') then
      raise invalid_parameter_value using message='Conclua o checklist de saída antes de liberar o veículo';
    end if;
