...
export function App({runtime=liveRuntime}:{runtime?:MobileRuntime}){const[page,setPage]=useState<Page>('home');const[snapshot,setSnapshot]=useState<MobileSnapshot|null>(null);const[booting,setBooting]=useState(true);const[error,setError]=useState('');const online=typeof navigator==='undefined'||navigator.onLine
 ...
 const canLoans=snapshot.scope.permissions.includes('loan.view')||snapshot.scope.permissions.includes('loan.request')||snapshot.scope.permissions.includes('loan.approve');const canOrders=snapshot.scope.permissions.includes('work_order.view')||snapshot.scope.permissions.includes('work_order.execute')||snapshot.scope.permissions.includes('work_order.create')
 ...
 if(page==='loans')content=<LoansHome loans={loans} vehicles={snapshot.vehicles} canRequest={snapshot.scope.permissions.includes('loan.request')} onNew={()=>setPage('loan-new')} onCalendar={()=>setPage('calendar')}/>
